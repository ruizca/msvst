...
void SplineAtrous::convol(fltarray &input, fltarray &output, int dim, int step)
{
    ...
    else if (dim == 3) // Z axis
    {
        ...
        throw DataException("SplineAtrous::convol: unknown axis");
}
    ...
    if (dim == 1)
    {
        ...
        if (ver2)
        {
            ...
            for (int i = 0; i < nlen; i++)
                detail(i) = data(i) - detail(i);
        }
        else
            for (int i = 0; i < nlen; i++)
                detail(i) = data(i) - appr(i);
    }
    else if (dim == 2)
    {
        float c = 1. / corr;
        ...
        if (ver2)
        {
            ...
            for (int i = 0; i < nlen; i++)
                detail(i) = c * data(i) - (*temp)(i);
            delete temp;
            temp = NULL;
        }
        else
            for (int i = 0; i < nlen; i++)
                detail(i) = c * data(i) - appr(i);
    }
    else if (dim == 3)
    {
        float c = 1. / (corr * corr);
        ...
        throw DataException("SplineAtrous::transform: unknown axis");
}
...
void SplineAtrous::recons(fltarray &appr, fltarray &detail, fltarray &data, int scale)
{
    ...
    else if (dim == 2)
    {
        ...
        if (ver2)
        {
            ...
            for (int i = 0; i < nlen; i++)
                data(i) = c * (data(i) + detail(i));
            delete temp;
            temp = NULL;
        }
        else
            for (int i = 0; i < nlen; i++)
                data(i) = c * (appr(i) + detail(i));
    }
    else if (dim == 3)
    {
        float c = corr * corr;
        ...
        throw DataException("SplineAtrous::recons: unknown axis");
}
#endif
