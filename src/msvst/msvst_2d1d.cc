...
static void filtinit(int argc, char *argv[])
{
  ...
  while ((c = GetOpt(argc, argv, "M:E:s:c:n:N:F:f:I:i:Q:B:TKpv")) != -1)
  {
    ...
    case 's':
      ...
      for (int i = 0; i < METHOD_LEN; i++)
        PROBA[i] = 2. * (1 - Utils<double>::cumNormal(GLEVEL));
      break;
  ...
  {
    cerr << "Error : Input image required." << endl;
    usage(argv);
    exit(-1);
  }
  ...
  {
    cerr << "Error : Output image required." << endl;
    usage(argv);
    exit(-1);
  }
...
void bandDenoise(fltarray &band, double sigma, to_array<SUPTYPE, true> *ms, int sxy, int sz, int d)
{
    ...
    for (int sz = 1; sz <= NSCALEZ; sz++)
    {
      ...
      *temp = ccxyz[sz];
      ...
      if ((sxy == NSCALEXY) && ((sxy >= FSCALEXY) || (sz >= FSCALEZ)))
      {
        ...
        bandDenoise<SUPTYPE>(*cdxyz, sigma, &multiSup[s], sxy, sz, 1);
        if ((sxy == NSCALEXY) && (KILLLAST))
          setConst(*cdxyz, *cdxyz, 0.);
        s++;
      }
    }
    ...
    for (int sz = 1; sz <= NSCALEZ; sz++)
    {
      ...
      if ((sxy >= FSCALEXY) && (sz >= FSCALEZ))
      {
        ...
        if (sz == NSCALEZ)
        {
          ...
          bandDenoise<SUPTYPE>(*dcxyz, sigma, &multiSup[s], sxy, sz, 2);
          s++;
        }
        // denoise ddxyz
        ...
        if ((sxy > 1) && (sz == 1))
            ...
            corr = 0.5;
        bandDenoise<SUPTYPE>(*ddxyz, corr * sigma, &multiSup[s], sxy, sz, 3);
        s++;
      }
    }

    // update ccpxyz
    for (int i = 0; i <= NSCALEZ; i++)
      ccpxyz[i] = ccxyz[i];
  }
...
void denoise(fltarray &data, fltarray *model)
{
  ...
  *origData = data;
  ...
  // char multiSup_filename[256];
  // sprintf(multiSup_filename, "%s_multisup.fits", Name_Imag_SNR);
  // fits_write_fltarr(multiSup_filename, *multiSup);
...
int main(int argc, char *argv[])
{
  ...
  if (VERBOSE)
  {
    ...
    if (MODELIM)
      cout << "Model image : " << Name_Imag_Model << endl;
  }
  ...
  if (VERBOSE)
  {
    ...
    {
      if (FDRINDEP)
        cout << "independence mode" << endl;
      else
        cout << "dependence mode" << endl;
    }
